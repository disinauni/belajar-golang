...
test.describe('Home page', () => {
  test('loads and shows hero section', async ({ page }) => {
    await page.goto('/id')
    await expect(page).toHaveTitle(/Belajar Golang/)
    await expect(page.locator('main h1').first()).toBeVisible()
  })

  test('EN version loads', async ({ page }) => {
    await page.goto('/en')
    await expect(page).toHaveTitle(/Learn Go/)
  })
...
test.describe('Curriculum page', () => {
  test('shows all 9 units', async ({ page }) => {
    await page.goto('/id/curriculum')
    await expect(page.locator('main h1').first()).toBeVisible()
    // Should have unit-0 through unit-8 visible
    const unitCards = page.locator('[data-unit-card]')
    await expect(unitCards).toHaveCount(9)
  })
})
