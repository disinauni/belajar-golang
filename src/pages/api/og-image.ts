...
export const GET: APIRoute = async ({ url }) => {
  const title = url.searchParams.get('title') || 'Belajar Golang'
  const description = url.searchParams.get('description') || 'Platform pembelajaran Go gratis dan interaktif'
  const lang = url.searchParams.get('lang') || 'id'

  const subtitle = lang === 'id' ? 'Platform Pembelajaran Go' : 'Go Learning Platform'
